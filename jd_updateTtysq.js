...
async function writeFile() {
    ...
    await fs.writeFileSync(`./shareCodes/ttysq.json`, JSON.stringify($.shareCoseList));
    console.log( `\n${JSON.stringify($.shareCoseList)}`)
                ...
                if (err) {
                    ...
                    if (function_name === "jxnhj/GetUserInfo") {
                        if (data.iRet === 0) {
                            ...
                            if (show === true) {
                                console.log(`等级：${$.level}，年货值：${$.goodsNum}，红包：${$.luckyNum}份，预计瓜分${$.luckyWorth}元，抽奖次数：${$.lotteryNum}次`)
